...
class ArticleForm(forms.ModelForm):
    ...
    class Meta:
        model = Article
        # fields = '__all__'  # Вообще-то так делать не стоит, чтобы не выводились поля запрещенные для редактирования
...
# Помимо того, что форма может быть автоматически сгенерирована на основе какой-либо модели, Django позволяет создавать
# их, просто указывая поля. Этот метод очень похож на то, как мы описываем модели. Для этого используется тело класса
# формы, а не вложенный в него Meta-класс.
